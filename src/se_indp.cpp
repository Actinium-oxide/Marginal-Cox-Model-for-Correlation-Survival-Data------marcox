...
// [[Rcpp::export]]
SEXP sandwich_indp_rcpp(double rho,
                       double betascale,
  ...
  for(int v=0; v<covnum; v++){
    for(int w=0; w<covnum; w++){
      for(int i_clust=0; i_clust<K; i_clust++){
        int ni = n[i_clust];
        MatrixXd Q1 = MatrixXd::Constant(ni, ni, 0);
        for(int d=0; d<ni; d++){
          Q1(d,d) = 1.0;
        }
        ...
        MatrixXd BB1 = outer_mat.array()*Q1.array();
        ...
        for(int j_idx=0; j_idx<ni; j_idx++){
          if(t21[j_idx]>=tt1[s]){
            ...
            for(int k=0; k<ni; k++){
              ...
              sum_val += colr[k]*sqrt_k*inv_sqrt_j*Q1(k,j_idx);
            }

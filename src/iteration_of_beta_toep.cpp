      ...
      while(true){
        ...
        MatrixXd V1 = MatrixXd::Zero(Kn, Kn);
        {
          ...
          for(int i=0; i<K; i++){
            ...
            MatrixXd R1 = absDiff.unaryExpr([rho](double d) -> double {
              int lag = static_cast<int>(d);
              return (lag == 0) ? 1.0 : rho(lag - 1);
            });
            R1.diagonal().setOnes();

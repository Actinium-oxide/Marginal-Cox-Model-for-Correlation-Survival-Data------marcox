...
VectorXd kernal(VectorXd dd_c, VectorXd g11_c, VectorXd t11_c, VectorXd tt1_c,
  ...
  int s2=1;
  int s3=1;
  VectorXd gSS=VectorXd::Zero(kk_c);
  VectorXd gSS1=VectorXd::Ones(kk_c);
  VectorXd gSSS1=VectorXd::Zero(kk_c);
  MatrixXd geebeta(xxx_c.cols(),1);
  int KK1;
  int min_ori=2147483646;
  VectorXd gSS2 (Kn_c);
  VectorXd gSS3 (Kn_c);
  ...
  VectorXd beta2 (xxx_c.cols());
  MatrixXd X1 (xxx_c.rows(),xxx_c.cols());
  VectorXd mu=VectorXd::Zero(xxx_c.rows());
  ...
  float pphi;
  int p=0;
  VectorXd temp_2 =VectorXd::Ones(n_c.size()-1);
  int SK=1;
  VectorXd mu_temp =VectorXd::Zero(int(n_c[0]));
  MatrixXd D1 (xxx_c.rows(),xxx_c.cols());
  VectorXd X1_2 =VectorXd::Ones(int(n_c[0]));
  int temp_3=0;
  VectorXd S1 (mu.size());
  ...
  while (s1==1){
    ...
    while(s2==1){
      ...
      while(s3==1){
        ...
        VectorXd temp_4 =VectorXd::Ones(int(n_c(j)));
